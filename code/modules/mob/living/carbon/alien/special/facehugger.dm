...
/obj/item/clothing/mask/facehugger/proc/Attach(mob/living/M)
	...
	if(attached)
		...
		spawn(MAX_IMPREGNATION_TIME)
			attached = 0
	if(M.get_organ(/obj/item/organ/internal/xenos/hivenode))
		return 0
	if(M.get_organ(/obj/item/organ/internal/body_egg/alien_embryo))
		return 0
	if(loc == M)
		return 0
	if(stat != CONSCIOUS)
...
/obj/item/clothing/mask/facehugger/proc/Impregnate(mob/living/target as mob)
	...
	if(!sterile)
		...
		if(!target.get_organ(/obj/item/organ/internal/body_egg/alien_embryo))
