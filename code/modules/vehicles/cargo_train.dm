...
// Standard procs
//-------------------------------------------
/obj/vehicle/train/med/engine/New()
	..()
	cell = new /obj/item/weapon/cell/high(src)
	key = new(src)
	var/image/I = new(icon = 'icons/obj/vehicles.dmi', icon_state = "docwagon2")
	I.plane = plane
	I.layer = layer
	overlays += I
	turn_off()	//so engine verbs are correctly set

/obj/vehicle/train/med/engine/Move(var/turf/destination)
	if(on && cell.charge < (charge_use * CELLRATE))
		turn_off()
		update_stats()
		if(load && is_train_head())
...
/obj/vehicle/train/med/on_update_icon()
	if(open)
		icon_state = initial(icon_state) + "_open"
	else
		icon_state = initial(icon_state)
...
/obj/vehicle/train/med/engine/remove_cell(var/mob/living/carbon/human/H)
	..()
	update_stats()

/obj/vehicle/train/med/engine/Bump(atom/Obstacle)
	var/obj/machinery/door/D = Obstacle
	var/mob/living/carbon/human/H = load
	if(istype(D) && istype(H))
		D.Bumped(H)		//a little hacky, but hey, it works, and respects access rights
...
// Train procs
//-------------------------------------------
/obj/vehicle/train/med/engine/turn_on()
	if(!key)
		return
	else
		..()
		...
		verbs -= /obj/vehicle/train/med/engine/verb/stop_engine
		verbs -= /obj/vehicle/train/med/engine/verb/start_engine
...
/obj/vehicle/train/med/engine/turn_off()
	..()
...
/obj/vehicle/train/med/RunOver(var/mob/living/carbon/human/H)
	var/list/parts = list(BP_HEAD, BP_CHEST, BP_L_LEG, BP_R_LEG, BP_L_ARM, BP_R_ARM)
...
// Interaction procs
//-------------------------------------------
/obj/vehicle/train/med/engine/relaymove(mob/user, direction)
...
/obj/vehicle/train/med/engine/examine(mob/user, distance)
	. = ..()
...
/obj/vehicle/train/med/engine/verb/start_engine()
	set name = "Start engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/med/engine/verb/stop_engine()
	set name = "Stop engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/med/engine/verb/remove_key()
	set name = "Remove key"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/med/engine/latch(obj/vehicle/train/T, mob/user)
	if(!istype(T) || !Adjacent(T))
		return 0

	//if we are attaching a trolley to an engine we don't care what direction
	// it is in and it should probably be attached with the engine in the lead
	if(istype(T, /obj/vehicle/train/med/trolley))
		T.attach_to(src, user)
	else
		var/T_dir = get_dir(src, T)	//figure out where T is wrt src
...
// engine.
//-------------------------------------------------------
/obj/vehicle/train/med/engine/update_car(var/train_length, var/active_engines)
	src.train_length = train_length
	src.active_engines = active_engines
...
/obj/vehicle/train/atv/engine
	name = "todo terreno"
	desc = "Un vehiculo todo terreno para aquellos lugares complicados."
	icon = 'icons/obj/vehicles.dmi'
	icon_state = "atv"
	on = 0
	powered = 1
	...
	charge_use = 1 KILOWATTS
	active_engines = 1
	var/obj/item/weapon/key/atv_train/key

/obj/item/weapon/key/atv_train
	name = "key"
	desc = "Las llave de un todo terreno."
	icon = 'icons/obj/vehicles.dmi'
	icon_state = "keydoc"
	w_class = ITEM_SIZE_TINY

/obj/vehicle/train/atv/trolley
	name = "camilla de ambulancia"
	icon = 'icons/obj/vehicles.dmi'
	icon_state = "ambulance"
	anchored = 1
...
// Standard procs
//-------------------------------------------
/obj/vehicle/train/atv/engine/New()
	..()
	cell = new /obj/item/weapon/cell/high(src)
	key = new(src)
	var/image/I = new(icon = 'icons/obj/vehicles.dmi', icon_state = "atv")
	I.plane = plane
	I.layer = layer
	overlays += I
	turn_off()	//so engine verbs are correctly set

/obj/vehicle/train/atv/engine/Move(var/turf/destination)
	if(on && cell.charge < (charge_use * CELLRATE))
		turn_off()
		update_stats()
		if(load && is_train_head())
...
/obj/vehicle/train/atv/trolley/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(open && isWirecutter(W))
		passenger_allowed = !passenger_allowed
		user.visible_message("<span class='notice'>[user] [passenger_allowed ? "cuts" : "mends"] a cable in [src].</span>","<span class='notice'>You [passenger_allowed ? "cut" : "mend"] the load limiter cable.</span>")
	else
...
/obj/vehicle/train/atv/engine/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/key/atv_train))
		if(!key)
			if(!user.unEquip(W, src))
				return
			key = W
			verbs += /obj/vehicle/train/atv/engine/verb/remove_key
		return
	..()

//cargo trains are open topped, so there is a chance the projectile will hit the mob ridding the train instead
/obj/vehicle/train/atv/bullet_act(var/obj/item/projectile/Proj)
	if(buckled_mob && prob(70))
		buckled_mob.bullet_act(Proj)
		return
	..()

/obj/vehicle/train/atv/on_update_icon()
	if(open)
		icon_state = initial(icon_state) + "_open"
	else
		icon_state = initial(icon_state)

/obj/vehicle/train/atv/trolley/insert_cell(var/obj/item/weapon/cell/C, var/mob/living/carbon/human/H)
	return

/obj/vehicle/train/atv/engine/insert_cell(var/obj/item/weapon/cell/C, var/mob/living/carbon/human/H)
	..()
	update_stats()

/obj/vehicle/train/atv/engine/remove_cell(var/mob/living/carbon/human/H)
	..()
	update_stats()

/obj/vehicle/train/atv/engine/Bump(atom/Obstacle)
	var/obj/machinery/door/D = Obstacle
	var/mob/living/carbon/human/H = load
	if(istype(D) && istype(H))
		D.Bumped(H)		//a little hacky, but hey, it works, and respects access rights
...
/obj/vehicle/train/atv/trolley/Bump(atom/Obstacle)
	if(!lead)
		return //so people can't knock others over by pushing a trolley around
	..()
...
// Train procs
//-------------------------------------------
/obj/vehicle/train/atv/engine/turn_on()
	if(!key)
		return
	else
		..()
		...
		verbs -= /obj/vehicle/train/atv/engine/verb/stop_engine
		verbs -= /obj/vehicle/train/atv/engine/verb/start_engine
...
/obj/vehicle/train/atv/engine/turn_off()
	..()
...
/obj/vehicle/train/atv/RunOver(var/mob/living/carbon/human/H)
	var/list/parts = list(BP_HEAD, BP_CHEST, BP_L_LEG, BP_R_LEG, BP_L_ARM, BP_R_ARM)
...
/obj/vehicle/train/atv/trolley/RunOver(var/mob/living/carbon/human/H)
	..()
	attack_log += text("\[[time_stamp()]\] <font color='red'>ran over [H.name] ([H.ckey])</font>")

/obj/vehicle/train/atv/engine/RunOver(var/mob/living/carbon/human/H)
	..()
...
// Interaction procs
//-------------------------------------------
/obj/vehicle/train/atv/engine/relaymove(mob/user, direction)
...
/obj/vehicle/train/atv/engine/examine(mob/user, distance)
	. = ..()
...
/obj/vehicle/train/atv/engine/verb/start_engine()
	set name = "Start engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/atv/engine/verb/stop_engine()
	set name = "Stop engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/atv/engine/verb/remove_key()
	set name = "Remove key"
	set category = "Object"
	set src in view(0)
...
// Loading/unloading procs
//-------------------------------------------
/obj/vehicle/train/atv/trolley/load(var/atom/movable/C)
	if(ismob(C) && !passenger_allowed)
		return 0
	if(!istype(C,/obj/structure/closet) && !istype(C, /mob/living/carbon/human))
		return 0
...
/obj/vehicle/train/atv/engine/load(var/atom/movable/C)
	if(!istype(C, /mob/living/carbon/human))
		return 0
...
// been unloaded. A dummy object is loaded instead so the loading
// code knows to handle it correctly.
/obj/vehicle/train/atv/trolley/proc/load_object(var/atom/movable/C)
	if(!isturf(C.loc)) //To prevent loading things from someone's inventory, which wouldn't get handled properly.
		return 0
	if(load || C.anchored)
		return 0
...
/obj/vehicle/train/atv/trolley/unload(var/mob/user, var/direction)
	if(istype(load, /datum/vehicle_dummy_load))
		var/datum/vehicle_dummy_load/dummy_load = load
		load = dummy_load.actual_load
		dummy_load.actual_load = null
...
/obj/vehicle/train/atv/engine/latch(obj/vehicle/train/T, mob/user)
	if(!istype(T) || !Adjacent(T))
		return 0

	//if we are attaching a trolley to an engine we don't care what direction
	// it is in and it should probably be attached with the engine in the lead
	if(istype(T, /obj/vehicle/train/atv/trolley))
		T.attach_to(src, user)
	else
		var/T_dir = get_dir(src, T)	//figure out where T is wrt src
...
// engine.
//-------------------------------------------------------
/obj/vehicle/train/atv/engine/update_car(var/train_length, var/active_engines)
	src.train_length = train_length
	src.active_engines = active_engines
...
/obj/vehicle/train/atv/trolley/update_car(var/train_length, var/active_engines)
	src.train_length = train_length
	src.active_engines = active_engines
...
/obj/vehicle/train/snow/engine
	name = "moto de nieve"
	desc = "Una moto de nieve para aquellos lugares nevados complicados."
	icon = 'icons/obj/vehicles.dmi'
	...
	charge_use = 1 KILOWATTS
	active_engines = 1
	var/obj/item/weapon/key/snow_train/key

/obj/item/weapon/key/snow_train
	name = "key"
	desc = "Las llave de un todo terreno."
	icon = 'icons/obj/vehicles.dmi'
	icon_state = "key"
...
// Standard procs
//-------------------------------------------
/obj/vehicle/train/snow/engine/New()
	..()
	cell = new /obj/item/weapon/cell/high(src)
	key = new(src)
	var/image/I = new(icon = 'icons/obj/vehicles.dmi', icon_state = "snowmobile")
	I.plane = plane
	I.layer = layer
	overlays += I
	turn_off()	//so engine verbs are correctly set

/obj/vehicle/train/snow/engine/Move(var/turf/destination)
	if(on && cell.charge < (charge_use * CELLRATE))
		turn_off()
		update_stats()
		if(load && is_train_head())
...
/obj/vehicle/train/snow/engine/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/key/snow_train))
		if(!key)
			if(!user.unEquip(W, src))
				return
			key = W
			verbs += /obj/vehicle/train/snow/engine/verb/remove_key
		return
	..()

//cargo trains are open topped, so there is a chance the projectile will hit the mob ridding the train instead
/obj/vehicle/train/snow/bullet_act(var/obj/item/projectile/Proj)
	if(buckled_mob && prob(70))
		buckled_mob.bullet_act(Proj)
		return
	..()

/obj/vehicle/train/snow/on_update_icon()
	if(open)
		icon_state = initial(icon_state) + "_open"
	else
		icon_state = initial(icon_state)

/obj/vehicle/train/snow/engine/insert_cell(var/obj/item/weapon/cell/C, var/mob/living/carbon/human/H)
	..()
	update_stats()

/obj/vehicle/train/snow/engine/remove_cell(var/mob/living/carbon/human/H)
	..()
	update_stats()

/obj/vehicle/train/snow/engine/Bump(atom/Obstacle)
	var/obj/machinery/door/D = Obstacle
	var/mob/living/carbon/human/H = load
	if(istype(D) && istype(H))
		D.Bumped(H)		//a little hacky, but hey, it works, and respects access rights
...
// Train procs
//-------------------------------------------
/obj/vehicle/train/snow/engine/turn_on()
	if(!key)
		return
	else
		..()
		...
		verbs -= /obj/vehicle/train/snow/engine/verb/stop_engine
		verbs -= /obj/vehicle/train/snow/engine/verb/start_engine
...
/obj/vehicle/train/snow/engine/turn_off()
	..()
...
/obj/vehicle/train/snow/RunOver(var/mob/living/carbon/human/H)
	var/list/parts = list(BP_HEAD, BP_CHEST, BP_L_LEG, BP_R_LEG, BP_L_ARM, BP_R_ARM)
...
/obj/vehicle/train/snow/engine/RunOver(var/mob/living/carbon/human/H)
	..()
...
// Interaction procs
//-------------------------------------------
/obj/vehicle/train/snow/engine/relaymove(mob/user, direction)
...
/obj/vehicle/train/snow/engine/examine(mob/user, distance)
	. = ..()
...
/obj/vehicle/train/snow/engine/verb/start_engine()
	set name = "Start engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/snow/engine/verb/stop_engine()
	set name = "Stop engine"
	set category = "Object"
	set src in view(0)
...
/obj/vehicle/train/snow/engine/verb/remove_key()
	set name = "Remove key"
	set category = "Object"
	set src in view(0)
...
// engine.
//-------------------------------------------------------
/obj/vehicle/train/snow/engine/update_car(var/train_length, var/active_engines)
	src.train_length = train_length
	src.active_engines = active_engines
